  ...
  const plans = [
    {
      ...
      features: [
        ...
        'No antri',
        'Bisa online',
        'Support 4K + 120 FPS (disesuaikan jaringan)',
        'Tanpa APK, cukup browser',
        '⭐ Free tutorial lengkap & gratis',
      ],
      pricing: [
        { duration: '1 Minggu (Premium)', price: '48K', subtitle: 'Bundling 4 akun' },
        { duration: '1 Bulan (Premium)', price: '138K', subtitle: 'Bundling 4 akun' },
        { duration: '2 Bulan (Premium)', price: '248K', subtitle: 'Bundling 4 akun' },
        { duration: '2 Bulan (Standard)', price: '118K', subtitle: 'Bundling 2 akun, 50-60 hari' },
      ],
      note: 'Premium: Sistem bundling 4 akun per minggu, dapat melanjutkan save FC. Standard: Game terbatas + FC26, wajib VPN sebelum ingame (bisa dimatikan setelah).',
      highlighted: true,
    },
    {
      id: 'nebula',
    ...
    },
    {
      ...
      name: 'Xbox Cloud',
      subtitle: 'BY NAIERSTORE',
      badge: 'BEST SELLER - UPDATE HARGA 29 JANUARI 2026',
      icon: '🎮',
                ...
                <div className="relative z-10 p-6 md:p-8 h-full flex flex-col">
                  ...
                  <div className="mb-6 space-y-2 border-t border-white/10 pt-4 flex-1">
                    ...
                    {plan.features.map((feature, j) => {
                      const isFreeFeature = feature.includes('Free tutorial')
                      return (
                        <div
                          key={j}
                          className={`flex items-start gap-2 rounded-lg px-2.5 py-1.5 text-xs md:text-sm group-hover:translate-x-1 transition-all duration-300 ${
                            isFreeFeature
                              ? 'bg-gradient-to-r from-yellow-500/20 to-amber-500/20 border border-yellow-500/40 shadow-lg shadow-yellow-500/20'
                              : ''
                          }`}
                          style={{ animation: `feature-slide-in 0.5s ease-out ${0.1 * j}s both` }}
                        >
                          <span className={`font-bold mt-0.5 flex-shrink-0 ${isFreeFeature ? 'text-yellow-400' : 'text-cyan-400'}`}>
                            {isFreeFeature ? '⭐' : '✓'}
                          </span>
                          <span className={`leading-snug ${isFreeFeature ? 'text-yellow-200 font-semibold' : 'text-white/85'}`}>
                            {feature}
                          </span>
                        </div>
                      )
                  ...
                  <div className="mb-4 space-y-2 border-t border-white/10 pt-4 w-full">
                    ...
                    <div className="max-h-48 overflow-y-auto space-y-1.5 pr-2">
                      {plan.pricing.map((price, j) => (
                        <div
                          key={j}
                          className="flex flex-col px-3 py-2 rounded-lg bg-white/5 hover:bg-white/10 smooth-transition text-xs md:text-sm border border-white/10 hover:border-white/30"
                        >
                          ...
                          {price.subtitle && (
                            <span className="text-white/60 text-xs mt-1">{price.subtitle}</span>
                          )}
                        </div>
                      ))}
                    </div>
